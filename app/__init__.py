from flask import Flask, render_template_string, request, render_template, abort,send_file, g, jsonify,send_from_directory,url_for, redirect,session
import subprocess
import sqlite3, os
...
def check_rate_limit():
    ...
    if "last_request_time" in session:
        ...
        if elapsed_time < TIME_WINDOW:
            ...
            if session["request_count"] > RATE_LIMIT:
...
def contact():
    ...
    blacklist = [".","[","]","_","join","init","flag",")","(","'","attr","<",">",'"',"/",'`',"|"]
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if result.returncode == 0:
                return render_template("result.html", result=result.stdout, command=" ".join(shlex.quote(arg) for arg in command))
            else:
                return render_template("result.html", result=result.stderr, command=" ".join(shlex.quote(arg) for arg in command))
        except Exception as e:
            return render_template("result.html", result=f"Error: {str(e)}", command=" ".join(shlex.quote(arg) for arg in command))
...
def view_file():
    ...
    if not filename:
        return "Имя файла не указано в запросе", 400  

  
    safe_filename = os.path.abspath(os.path.join(app.config['UPLOAD_FOLDER'], filename))

 
    if not safe_filename.startswith(app.config['UPLOAD_FOLDER'] + os.path.sep):
        abort(404)  
    try:
...
def loginAPI():
    if request.method == 'POST':
        ...
        g.db = connect_db()
        cur = g.db.execute("SELECT * FROM employees WHERE username = ? AND password = ?", (uname, pword))
        if cur.fetchone():
...
def admin_panel():
    ...
    if user_param == 'f354e988-2dee-4100-ad19-963b01258882':
